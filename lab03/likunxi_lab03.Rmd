...
p3 <- mich + theme_void() + 
  geom_polygon(data = mich_county, fill = NA, color = "white") +
  geom_polygon(color = "black", fill = NA) +
  geom_point(data = animateMichNotNature, aes(longitude, latitude), inherit.aes = FALSE) +
  # Plot the historical mean points with geom_path before the transition
...
combined_df <- bind_rows(variance_nature, variance_notNature)

# Now, plot the combined data frame
# Plot variance of longitude
ggplot(combined_df, aes(x = month, y = var_lon, color = type)) +
  geom_line() +
...
ggplot(combined_df, aes(x = month, y = var_lat, color = type)) +
  geom_line() +
...
```{r}
library(terra)
SL_2001<-as.factor(rast("SaltlakeData/NLCD_2001_SL.tif"))
SL_2004<-as.factor(rast("SaltlakeData/NLCD_2004_SL.tif"))
SL_2006<-as.factor(rast("SaltlakeData/NLCD_2006_SL.tif"))
SL_2008<-as.factor(rast("SaltlakeData/NLCD_2008_SL.tif"))
SL_2011<-as.factor(rast("SaltlakeData/NLCD_2011_SL.tif"))
SL_2013<-as.factor(rast("SaltlakeData/NLCD_2013_SL.tif"))
SL_2016<-as.factor(rast("SaltlakeData/NLCD_2016_SL.tif"))
plot(SL_2001)
```
